...
function deletePatient(patientId) {
  ...
  if (updatedPatientList.length < patientList.length) {
...
function renderPatientList() {
  ...
  patientList.forEach(function (patient) {
    ...
    listItem.innerHTML = `
      ...
      (Idade: ${patient.age})
      <button class="delete-button" onclick="deletePatient(${patient.id})">
        Excluir
      </button>
    `;
...
/* ============================================================================
   o trecho de código abaixo é uma versão anterior da função renderPatientList, 
   que utiliza concatenação de strings para construir o conteúdo HTML. 
   Ele foi substituído pela versão atual que utiliza template literals (strings template)
   ============================================================================
// Função para renderizar a lista de pacientes no HTML
function renderPatientList() {
  var patientListElement = document.getElementById('patientList');
...
/* ============================================================================
   o trecho de código abaixo é uma versão anterior da função renderPatientList, 
   que utiliza um loop for tradicional para iterar sobre a lista de pacientes. 
   Ele foi substituído pela versão atual que utiliza o método forEach 
   ============================================================================
for (var i = 0; i < patientList.length; i++) {
  var patient = patientList[i]; // pega o paciente atual
  var listItem = document.createElement('li');
  listItem.innerHTML =
    '<span class="patient-name">' + patient.name + '</span> (Idade: ' +
    patient.age +
    ') <button class="delete-button" onclick="deletePatient(' +
    patient.id +
    ')">Excluir</button>';
  patientListElement.appendChild(listItem);
}
